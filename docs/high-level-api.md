...
The useful `%%` syntax means SBT will select the appropriate binary for your Scala version, and `1.5` is the
...
(see [build.sbt](https://github.com/chrisnappin/play-recaptcha-example/blob/play-2.4-branch/build.sbt) for a complete example)
...
(see [application.conf](https://github.com/chrisnappin/play-recaptcha-v2-example/blob/play-2.4-branch/conf/application.conf) for a complete example)
...
(see [application.conf](https://github.com/chrisnappin/play-recaptcha-example/blob/play-2.4-branch/conf/application.conf) for a complete example)
...
The [play-recaptcha v2 example application](https://github.com/chrisnappin/play-recaptcha-v2-example/tree/release-1.5) is internationalised to two languages (English, French) as an example.
...
The [play-recaptcha example application](https://github.com/chrisnappin/play-recaptcha-example/tree/release-1.5) is internationalised to two languages (English, French) as an example.
...
In your view template, you need to include a `recaptcha.recaptchaField` view helper tag within a `form` tag. This will render all of the JavaScript and HTML required for reCAPTCHA, plus by default the `noscript` option for browsers with JavaScript turned off (typically rare these days). Here is a very simple example:
    ...
    @(myForm: Form[MyModelObject])(implicit request: Request[AnyContent], messages: Messages)
...
(see [form.scala.html](https://github.com/chrisnappin/play-recaptcha-example/blob/play-2.4-branch/app/views/form.scala.html) for a complete example)
...
(see [messages](https://github.com/chrisnappin/play-recaptcha-example/blob/play-2.4-branch/conf/messages) for a complete example)
...
(see [ExampleForm.scala](https://github.com/chrisnappin/play-recaptcha-example/blob/play-2.4-branch/app/controllers/ExampleForm.scala) for a complete example)


##Troubleshooting
As mentioned above, the play-recaptcha module checks the application configuration upon startup, and if it finds some fatal problems it will refuse to startup. If this happens you should see a message like the following in your application logs:
